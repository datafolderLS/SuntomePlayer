...
func _ready() -> void:
	for obj_name in SuntomeGlobal.sound_object_contents:
		var obj = SuntomeGlobal.sound_object_contents[obj_name]
		var index = add_new_obj()
		itemlist.set_item_text(index, obj.name)
...
# 	for obj in SuntomeGlobal.sound_object_contents:
# 		var index = add_new_obj()
# 		itemlist.set_item_text(index, obj.name)
# 		pass
# 	pass
...
func start_edit_obj_name(index : int):
	...
	print("start edit sound obj in index :", index, " name: ", text)
...
func confirm_edit_obj_name(index : int, new_text: String):
	...
	print("confim sound obj name to ", new_text)

	_update_sound_obj_content_name(preName, new_text)
...
func create_new_obj_content():
	var i = add_new_obj()
	_add_new_sound_obj_content(itemlist.get_item_text(i))


#在itemlist中添加一个新对象，并返回该对象在itemlist中的下标
func add_new_obj() -> int:
	var nameList = get_all_obj_names()
	var new_text = "new sound object"
	new_text = Utility.check_no_repeat_name(new_text, nameList)
...
func remove_button_clicked():
	...
	_remove_sound_obj_content(name)
...
func _add_new_sound_obj_content(name : String):
	var new_cont = SoundObjContent.new()
	new_cont.name = name
	SuntomeGlobal.sound_object_contents[name] = new_cont
	pass


func _update_sound_obj_content_name(name_before : String, name_after : String):
...
func _remove_sound_obj_content(name : String):
	var needDelete = SuntomeGlobal.sound_object_contents[name]
	needDelete.before_delete()
	SuntomeGlobal.sound_object_contents.erase(name)
